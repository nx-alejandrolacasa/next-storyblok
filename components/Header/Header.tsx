...
export async function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-screen bg-white">
      <div className="mx-auto flex h-20 max-w-(--breakpoint-xl) items-center justify-between px-8">
        ...
        <ul className="flex flex-row gap-4 border-l border-l-sky-500 pl-4">
          {routing.locales.map((lang) => (
            <li key={lang}>
              {locale === lang ? (
                <span className="text-sky-500">{lang.toUpperCase()}</span>
              ) : (
                <Link
                  className="transition-all hover:text-sky-500 hover:underline"
                  href="/"
